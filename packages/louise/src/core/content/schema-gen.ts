  ...
  switch (field.type) {
    ...
    default:
      throw new LouiseContentError(
        `Field type "${(field as FieldConfig).type}" is not yet supported by louise/content schema-gen`,
      );
  }
}
...
export function generateSchemaSource(
  ...
  return [
    ...
    ...(needsJsonValue ? ['import type { JsonValue } from "louise/content";'] : []),
