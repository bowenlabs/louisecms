...
you hand-assemble. You call [`mountDrawer`](/docs/reference/client/#louisecmsclientdrawer)
with a config, and the shell renders the chrome, the fixed framework panels, and
your registered collection tabs. It pairs with the [`louisecms/editor`](/docs/reference/editor/)
...
- **Top strip — fixed framework panels.** **Pages**, **Media**, and **Settings**,
  ...
  [`louisecms/editor`](/docs/reference/editor/) endpoints (`/api/louise/pages`,
...
(mapped 1:1 to [`siteSettingsColumns`](/docs/reference/db/) — identity,
...
`site_settings.custom` JSON via the [`settings` handler](/docs/reference/editor/)'s
site-declared keys:
...
[data layer](/docs/reference/client/#louisecmsclientdrawer) and the shared field
...
identically. See the [client reference](/docs/reference/client/) for the full
export list.
